...
export const propertyService = {
  createProperty: async (data, user) => {
    if (user.role === "LANDLORD") {
      ...
      // landlordId is optional for admins
      if (data.landlordId) {
        const landlord = await propertyRepository.findUserById(data.landlordId);
        if (!landlord || landlord.role !== "LANDLORD") {
          throw new AppError("Invalid landlord ID", 400);
      ...
      // if no landlordId provided → remains null (unassigned)
    }
  ...
  assignLandlord: async (propertyId, landlordId) => {
    ...
    if (property.landlordId) {
      throw new AppError("Property already assigned", 400);
    }
    ...
    return await propertyRepository.updatePropertyLandlordId(propertyId, landlordId);
  },
